hospitalApp.directive('fullTable', ['$compile', 'globalData', function($compile, globalData) {
	return {
		replace: true,
		//templateUrl: '../js/directives/templates/fullTable.html',
		template: '<table class="table table-no-borders" isolate-form></table>',
		scope: {
			table: '=',
			thead: '@'
		},
		controller: ['$scope', '$element',
			function($scope, $element) {
				...
				angular.forEach($scope.table.config.fields, function(fieldOptions, fieldKey) {
					...
					if(fieldOptions.itemsName) options.itemsName = fieldOptions.itemsName;
					if(fieldOptions.width) options.width = fieldOptions.width;

					tempHtml+= '<td style="width:' + percent_width + '">';
					tempHtml+= '<field type="'
					           + fieldOptions.type
					           + '" value="newItem.'
					           + fieldKey
					           + '" options="' + angular.toJson(options).replace(/"/g, "'") + '">';

					if(fieldOptions.addon) tempHtml+= fieldOptions.addon;
